#!/usr/bin/env python3
"""Run all Stage 48B validators (includes 48A/47F regression pack)."""
...
def _check_build_stage() -> list[str]:
    failed = []
    for rel, needle in (
        ('backend/config/local_safe_mode.py', 'AstraEdge 48B'),
        ('backend/telegram/telegram_analysis_bot.py', 'AstraEdge 48B'),
        ('backend/telegram/response_format.py', 'AstraEdge 48B'),
        ('backend/api/api_server.py', "'stage': '48B'"),
        ('backend/analytics/budget_impact.py', "STAGE = '48B'"),
        ('backend/analytics/premarket_conviction.py', "'stage': '48B'"),
    ):
...
def main() -> int:
    ...
    if build_fail:
        for item in build_fail:
            print(f'STAGE_48B_FAIL: {item}', file=sys.stderr)
        return 1
    ...
    if failed:
        print(f'STAGE_48B_FAIL: {", ".join(failed)}', file=sys.stderr)
        return 1
    print('STAGE_48B_OK')
    return 0
