...
def send_message_result(text, parse_mode='HTML', disable_preview=True) -> dict:
    ...
    try:
        ...
        return {'ok': False, 'sent': False, 'skipped': False, 'reason': f'http_{response.status_code}'}
...
def send_message(text, parse_mode='HTML', disable_preview=True):
    ...
    """
    return bool(send_message_result(text, parse_mode=parse_mode, disable_preview=disable_preview).get('sent'))
