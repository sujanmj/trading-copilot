#!/usr/bin/env python3
"""Run all Stage 46J validators (includes 46I regression pack)."""
...
def main() -> int:
    failed = []
    for name in VALIDATORS_46J + VALIDATORS_46I + VALIDATORS_46H:
        ...
        if os.system(f'{sys.executable} scripts/{name}') != 0:
            failed.append(name)
    if failed:
        print(f'STAGE_46J_FAIL: {", ".join(failed)}', file=sys.stderr)
        return 1
    print('STAGE_46J_OK')
    return 0
