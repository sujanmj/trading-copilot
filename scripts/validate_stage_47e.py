#!/usr/bin/env python3
"""Run all Stage 47E validators (includes 47D regression pack)."""
...
def _check_build_stage() -> list[str]:
    failed = []
    for rel, needle in (
        ('backend/config/local_safe_mode.py', 'AstraEdge 47E'),
        ('backend/telegram/telegram_analysis_bot.py', 'AstraEdge 47E'),
        ('backend/telegram/response_format.py', 'AstraEdge 47E'),
        ('backend/api/api_server.py', "'stage': '47E'"),
        ('backend/analytics/theme_baskets.py', "STAGE = '47E'"),
        ('backend/analytics/premarket_conviction.py', "'stage': '47E'"),
    ):
...
def main() -> int:
    ...
    if failed:
        print(f'STAGE_47E_FAIL: {", ".join(failed)}', file=sys.stderr)
        return 1
    print('STAGE_47E_OK')
    return 0
