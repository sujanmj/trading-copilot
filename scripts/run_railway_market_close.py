#!/usr/bin/env python3
"""Railway cron: market close summary — one shot, exit (Stage 46A)."""
...
def main() -> int:
    ...
    from backend.telegram.telegram_brief_scheduler import build_close_brief_text, send_brief

    token_set = bool(os.environ.get('TELEGRAM_BOT_TOKEN', '').strip())
    chat_set = bool(os.environ.get('TELEGRAM_CHAT_ID', '').strip())
    print('[RAILWAY_CRON] job=market_close', flush=True)
    ...
    if not token_set or not chat_set:
        preview = build_close_brief_text()
