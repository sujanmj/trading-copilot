#!/usr/bin/env python3
"""Stage 50R — /full Step 09 /today must match direct /today output."""
...
def _fail(msg: str) -> int:
    print(f'FULL_TODAY_MATCHES_DIRECT_TODAY_TEST_FAIL: {msg}', file=sys.stderr)
    return 1
...
def main() -> int:
    ...
    with ExitStack() as stack:
        ...
        try:
            ...
            full_step = _handle_stock_decision_command('today', cache_only=True)
        finally:
            end_unified_snapshot()

    if 'AVANTIFEED' in full_step:
        return _fail('/full Step 09 still shows stale AVANTIFEED')
    if 'DEVYANI' not in direct and 'NO VALID ENTRY NOW' not in direct:
        return _fail(f'direct /today missing DEVYANI or no-entry state: {direct[:120]!r}')
    if direct.strip() != full_step.strip():
        return _fail('/full Step 09 body must match direct /today exactly')
    if 'AVANTIFEED' in direct:
        return _fail('direct /today must not show stale AVANTIFEED')

    print('FULL_TODAY_MATCHES_DIRECT_TODAY_TEST_OK')
    return 0
