...
def local_log_once(key: str, tag: str, message: str) -> None:
    """Log a message at most once per process (for repeated stale/verify noise)."""
    if key in _LOGGED_ONCE:
        return
    _LOGGED_ONCE.add(key)
    local_log(tag, message)
...
def verify_api_endpoints(*, verbose: bool = True) -> Dict[str, Any]:
...
def verify_gui_payload(
    ...
    *,
    ...
    for name, passed in checks.items():
        label = 'OK' if passed else 'EMPTY/STALE'
        if verbose:
            local_log('GUI VERIFY', f'{name}: {label}')
        elif not passed:
            if (
                LOCAL_QUIET_MODE
                and api_healthy
                ...
                local_log_once('gui-lifecycle-stale', 'GUI VERIFY', f'{name}: {label}')
            else:
                local_log('GUI VERIFY', f'{name}: {label}')
    return {'ok': ok, 'checks': checks, 'panels': panels}


def verify_lifecycle(*, verbose: bool = True, api_healthy: bool = False) -> Dict[str, Any]:
    try:
        ...
        msg = f'pipeline_status={status} complete={complete}'
        if verbose:
            local_log('LIFECYCLE VERIFY', msg)
        elif not complete:
            if LOCAL_QUIET_MODE and api_healthy:
                local_log_once(f'lifecycle-stale-{status}', 'LIFECYCLE VERIFY', msg)
            else:
                local_log('LIFECYCLE VERIFY', msg)
...
def verify_opportunities(limit: int = 20, *, verbose: bool = True) -> Dict[str, Any]:
    try:
        ...
        if verbose:
            ...
            local_log('API VERIFY', f'/opps ranked count={len(opps)} exceeds max {limit}')
...
def run_local_validation_loop(*, max_rounds: int = 30, interval: int = 20) -> None:
    ...
    def _loop():
        ...
        for round_num in range(1, max_rounds + 1):
            verbose = (round_num == 1) or not LOCAL_QUIET_MODE
            if verbose:
                local_log('LOCAL RUNTIME', f'validation round {round_num}/{max_rounds}')
            elif LOCAL_QUIET_MODE and round_num % 10 == 0:
                local_log('LOCAL RUNTIME', f'validation heartbeat round {round_num}/{max_rounds}')

            api = verify_api_endpoints(verbose=verbose)
            api_healthy = api.get('ok', False)
            lc = verify_lifecycle(verbose=verbose, api_healthy=api_healthy)
            opps = verify_opportunities(verbose=verbose)
            snap = None
            try:
                snap = fetch_json('/api/runtime/snapshot')
            except Exception:
                pass
            gui = verify_gui_payload(snap, verbose=verbose, api_healthy=api_healthy)
