#!/usr/bin/env python3
"""Run all Stage 47D validators (includes 47C regression pack)."""
...
def _check_build_stage() -> list[str]:
    failed = []
    for rel, needle in (
        ('backend/config/local_safe_mode.py', 'AstraEdge 47D'),
        ('backend/telegram/telegram_analysis_bot.py', 'AstraEdge 47D'),
        ('backend/telegram/response_format.py', 'AstraEdge 47D'),
        ('backend/api/api_server.py', "'stage': '47D'"),
    ):
...
def main() -> int:
    ...
    if failed:
        print(f'STAGE_47D_FAIL: {", ".join(failed)}', file=sys.stderr)
        return 1
    print('STAGE_47D_OK')
    return 0
