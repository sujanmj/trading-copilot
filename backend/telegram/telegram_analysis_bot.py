...
def parse_command(text: str) -> tuple[str, str]:
    ...
    if lower.startswith('pattern '):
        return 'pattern', raw[len('pattern '):].strip()
    if lower == 'pattern':
        return 'pattern', ''
...
def handle_analysis_command(
    ...
    *,
    ...
    if cmd in ('start', 'help', 'h', 'commands'):
        ...
        result = run_without_ai(lambda: run_pattern_only(args), command='pattern')
        response_text = result.get('text') or 'Pattern pick unavailable.'
    elif cmd == 'candles':
        ...
        result = run_without_ai(lambda: run_catalyst_only(args), command='catalyst')
        response_text = result.get('text') or 'Catalyst lookup unavailable.'
    elif cmd == 'catalysts':
