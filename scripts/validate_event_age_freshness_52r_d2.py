...
def _validate_changed_file_scope() -> str | None:
    ...
    if actual_head != CANONICAL_HEAD:
        return f'HEAD must remain canonical D2P production baseline {CANONICAL_HEAD}'
...
def main() -> int:
    ...
    if (BUILD_STAGE, TELEGRAM_BUILD) != ('52R-D2', 'AstraEdge 52R-D2'):
        return _fail(
            f'build must be exact 52R-D2 / AstraEdge 52R-D2, got {BUILD_STAGE!r} / {TELEGRAM_BUILD!r}'
        )
