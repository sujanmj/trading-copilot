...
def main() -> int:
    ...
    if 'Telegram build: <code>AstraEdge 49C</code>' not in status_text:
        return _fail('/status missing AstraEdge 49C build label')

    health_text = _handle_health()
    if 'Telegram build: <code>AstraEdge 49C</code>' not in health_text:
        return _fail('/health missing AstraEdge 49C build label')
