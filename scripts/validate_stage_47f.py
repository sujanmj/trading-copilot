#!/usr/bin/env python3
"""Run all Stage 47F validators (includes 47E regression pack)."""
...
def _check_build_stage() -> list[str]:
    failed = []
    for rel, needle in (
        ('backend/config/local_safe_mode.py', 'AstraEdge 47F'),
        ('backend/telegram/telegram_analysis_bot.py', 'AstraEdge 47F'),
        ('backend/telegram/response_format.py', 'AstraEdge 47F'),
        ('backend/api/api_server.py', "'stage': '47F'"),
        ('backend/analytics/theme_baskets.py', "STAGE = '47F'"),
        ('backend/analytics/premarket_conviction.py', "'stage': '47F'"),
    ):
...
def main() -> int:
    ...
    if failed:
        print(f'STAGE_47F_FAIL: {", ".join(failed)}', file=sys.stderr)
        return 1
    print('STAGE_47F_OK')
    return 0
