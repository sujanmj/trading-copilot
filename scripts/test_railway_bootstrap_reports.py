#!/usr/bin/env python3
"""
Unit tests for Railway live-data bootstrap reports (Stage 46F).

Usage:
  python scripts/test_railway_bootstrap_reports.py

Prints RAILWAY_BOOTSTRAP_REPORTS_TEST_OK on success.
"""
...
def _fail(msg: str) -> int:
    print(f'RAILWAY_BOOTSTRAP_REPORTS_TEST_FAIL: {msg}', file=sys.stderr)
    return 1
...
def test_module_wiring() -> str | None:
    ...
    for fragment in (
        ...
        'stock_decision_cache_ready',
        'RAILWAY_BOOTSTRAP_REPORTS_OK',
        'No clean candidate. Use /aihub market and /news.',
        'Bootstrap started. Try /today again in 1–2 minutes.',
        'Decision reports are being rebuilt. Try /today again in 1–2 minutes.',
    ):
        if fragment not in src:
    ...
    if 'cmd == \'bootstrap\'' not in bot and 'cmd == "bootstrap"' not in bot:
        return 'telegram bot missing /bootstrap handler'
    if '_handle_stock_decision_command' not in bot:
        return 'telegram bot missing lazy stock decision handler'
    return None
...
def test_aihub_brain_fallback() -> str | None:
    ...
    if 'stock_decision_today' not in src:
        return 'build_brain_payload missing stock_decision_today summary'
...
def test_bootstrap_skip_fresh() -> str | None:
    ...
    if mod.report_cache_needs_bootstrap():
        return None
    result = mod.run_railway_bootstrap_reports(
        force=False,
        timeout_sec=5,
        railway_only=False,
    )
...
def main() -> int:
    ...
    print(STAGE_MARKER)
    print('RAILWAY_BOOTSTRAP_REPORTS_TEST_OK')
    return 0
