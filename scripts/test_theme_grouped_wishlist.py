#!/usr/bin/env python3
"""Unit tests for grouped Theme Wishlist list output (Stage 47C)."""
...
def _fail(msg: str) -> int:
    print(f'THEME_GROUPED_WISHLIST_TEST_FAIL: {msg}', file=sys.stderr)
    return 1
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if 'Theme Wishlist' not in help_src:
            return _fail('/help missing Theme Wishlist section')
        if help_src.count('alias') > 2:
            return _fail('/help has alias spam')

    print('THEME_GROUPED_WISHLIST_TEST_OK')
    return 0
