...
ALLOWED_HEADS = frozenset({CANONICAL_HEAD, COMMITTED_53A2_HEAD, COMMITTED_53B_HEAD})
...
def main() -> int:
    ...
    if (BUILD_STAGE, TELEGRAM_BUILD) not in {
        ...
        return _fail(
            f'build must be exact 53A2 or successor 53B/53C pair, got {BUILD_STAGE!r} / {TELEGRAM_BUILD!r}'
        )
