...
def main() -> int:
    for rel, needle in (
        ('backend/config/local_safe_mode.py', 'AstraEdge 48F'),
        ('backend/api/api_server.py', "'stage': '48F'"),
        ('backend/analytics/budget_impact.py', "STAGE = '48F'"),
        ('backend/telegram/response_format.py', 'AstraEdge 48F'),
    ):
        if needle not in (PROJECT_ROOT / rel).read_text(encoding='utf-8'):
            print(f'STAGE_48F_FAIL: {rel} missing build', file=sys.stderr)
            return 1
    failed = []
    for name in VALIDATORS:
        ...
        if os.system(f'{sys.executable} scripts/{name}') != 0:
            failed.append(name)
    if failed:
        print(f'STAGE_48F_FAIL: {", ".join(failed)}', file=sys.stderr)
        return 1
    print('STAGE_48F_OK')
    return 0
