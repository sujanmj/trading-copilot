...
v3 Changes (from v2):
  - Added 8 new brain commands that trigger telegram_brain_pusher.py
  - Updated /help with full menu


v2 Changes (from v1):
  - Long-running commands run in daemon threads
  - cmd_ask() uses direct import (no subprocess injection risk)
  - Added is_silenced() helper
...
def _do_refresh():
    ...
    send_message("✅ <b>Refresh complete!</b>\n\nFull brain sent above. Use /opps for opportunities or /scan for top signals.")
...
<b>🧠 BRAIN (auto-pushed after each strategic run):</b>
/brain - Full 6-message brain analysis
...
def listen_forever():
    safe_print("=" * 60)
    safe_print("TELEGRAM LISTENER v3 - Path H (Brain Commands Added)")
