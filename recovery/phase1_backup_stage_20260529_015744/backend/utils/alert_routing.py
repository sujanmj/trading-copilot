...
def send_operational_telegram(message: str, *, severity: str, code: str) -> bool:
    from backend.utils.telegram_guard import guard_telegram_send
    if not guard_telegram_send('alert_routing.send_operational_telegram'):
        return True
...
def emit_operational_alert(
    ...
    *,
    ...
    if decision.get('telegram'):
        send_operational_telegram(message, severity=severity, code=code)
    return decision
