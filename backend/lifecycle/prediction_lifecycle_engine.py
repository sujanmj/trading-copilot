...
from backend.storage.db_manager import get_connection, get_trading_db_path, init_db
...
def _trading_predictions_schema_ok(conn: sqlite3.Connection) -> bool:
    cols = {
        row[1]
        for row in conn.execute("PRAGMA table_info(predictions)").fetchall()
    }
    return 'id' in cols and 'prediction_date' in cols


def _guard_trading_db_schema(conn: sqlite3.Connection) -> bool:
    if _trading_predictions_schema_ok(conn):
        return True
    path = get_trading_db_path()
    print(
        f"[LIFECYCLE] wrong DB schema detected; expected trading_history.db, got {path}"
    )
    return False
...
def expire_stale_pending() -> dict:
    ...
    try:
        ...
        conn = get_connection()
        try:
            if not _guard_trading_db_schema(conn):
                return stats
