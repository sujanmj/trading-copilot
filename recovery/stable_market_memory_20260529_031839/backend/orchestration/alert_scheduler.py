...
def tick() -> dict:
    ...
    now = _now_ist()
    result = {'time': now.isoformat(), 'sent': 0, 'skipped': 0, 'mode': 'market', 'actions': []}

    def _apply_emergency(sent: int, skipped: int) -> None:
        result['sent'] += sent
    ...
    if not is_weekday(now):
        ...
        if is_night_mode(now) and _once_per_slot('emergency_weekend', now, 15):
            _apply_emergency(*engine.try_emergency_macro())
        return result
