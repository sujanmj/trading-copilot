#!/usr/bin/env python3
"""Run all Stage 47B validators (includes 47A regression pack)."""
...
def main() -> int:
    failed = []
    for name in VALIDATORS_47B + VALIDATORS_47A + VALIDATORS_46J + VALIDATORS_46I + VALIDATORS_46H:
        ...
        if os.system(f'{sys.executable} scripts/{name}') != 0:
            failed.append(name)
    if failed:
        print(f'STAGE_47B_FAIL: {", ".join(failed)}', file=sys.stderr)
        return 1
    print('STAGE_47B_OK')
    return 0
